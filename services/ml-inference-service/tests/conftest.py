...
def mock_triton_client(mocker):
    ...
    async def mock_batch_predict_prices(features_batch, model_version=None):
        ...
        for i, features in enumerate(features_batch):
            last_price = features[-1, 0] if len(features) > 0 else 71000.0
            results.append(
                {"predicted_price": float(last_price * 1.01), "batch_index": i, "model_version": model_version or "1"}
            )
        return results
    ...
    async def mock_get_model_metadata(model_name):
        return {
            ...
            "inputs": [{"name": "input__0", "datatype": "FP32", "shape": [-1, 60, 5]}],
    ...
    async def mock_get_server_metrics():
        return {
            "lstm_price_predictor": {"count": 100, "total_time_ns": 1250000000, "avg_time_ms": 12.5, "version": "1"}
        }
...
def performance_metrics():
    ...
    return {
        "latency_ms": {"mean": 12.5, "median": 11.0, "p95": 18.0, "p99": 22.0, "min": 8.0, "max": 35.0},
        "throughput": {
