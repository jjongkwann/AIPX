...
class TestInferenceLatency:
    ...
    async def test_ensemble_inference_latency(self, mock_triton_client, sample_features, sample_tokenized_input):
        ...
        for _ in range(n_iterations):
            start_time = time.perf_counter()
            await mock_triton_client.ensemble_predict(sample_features, input_ids, attention_mask)
            end_time = time.perf_counter()
    ...
    async def test_batch_inference_latency(self, mock_triton_client):
        ...
        for batch_size in batch_sizes:
            features_batch = [np.random.randn(60, 5).astype(np.float32) for _ in range(batch_size)]
            ...
            results[batch_size] = {"avg_total_ms": avg_latency, "avg_per_sample_ms": avg_per_sample}

        print("\nBatch Inference Latency:")
        for batch_size, metrics in results.items():
            print(
                f"  Batch {batch_size}: {metrics['avg_total_ms']:.2f}ms total, "
...
class TestInferenceBenchmark:
    ...
    def test_feature_extraction_benchmark(self, benchmark, sample_market_data):
        ...
        result = benchmark(extractor.extract_price_features, sample_market_data, 60)
