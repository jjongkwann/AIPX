...
class TestThroughput:
    ...
    async def test_lstm_throughput(self, mock_triton_client, sample_features):
        ...
        tasks = [mock_triton_client.predict_price(sample_features) for _ in range(n_requests)]
    ...
    async def test_sentiment_throughput(self, mock_triton_client, sample_tokenized_input):
        ...
        tasks = [mock_triton_client.analyze_sentiment(input_ids, attention_mask) for _ in range(n_requests)]
    ...
    async def test_batch_throughput(self, mock_triton_client):
        ...
        for batch_size in batch_sizes:
            ...
            features_batch = [
                sample_features for sample_features in [pytest.lazy_fixture("sample_features")] * batch_size
            ]
            ...
            tasks = [mock_triton_client.batch_predict_prices(features_batch) for _ in range(n_batches)]
            ...
            results[batch_size] = {"total_samples": total_samples, "duration": duration, "throughput": throughput}
...
class TestConcurrency:
    ...
    async def test_concurrent_clients(self, mock_triton_client, sample_features):
        ...
        for n in n_clients:
            ...
            results[n] = {"total_requests": total_requests, "duration": duration, "throughput": throughput}
...
class TestResourceUtilization:
    ...
    async def test_cpu_utilization(self, mock_triton_client, sample_features):
        ...
        # Run inferences
        tasks = [mock_triton_client.predict_price(sample_features) for _ in range(1000)]
        await asyncio.gather(*tasks)
...
class TestScalability:
    ...
    async def test_linear_scalability(self, mock_triton_client, sample_features):
        ...
        for n_requests in load_levels:
            ...
            results[n_requests] = {"duration": duration, "throughput": throughput}
