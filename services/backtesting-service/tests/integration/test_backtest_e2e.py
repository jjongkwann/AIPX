...
from src.engine.matching_engine import MatchingEngine, Order, OrderType, OrderSide
...
class TestBacktestEndToEnd:
    ...
    def test_backtest_with_multiple_symbols(self, market_data_generator):
        ...
        data1 = market_data_generator(
            symbol='005930',
            initial_price=70000,
            seed=42
        )
        data2 = market_data_generator(
            symbol='000660',
        ...
        strategies = {
            '005930': SimpleMomentumStrategy(lookback=20),
            '000660': SimpleMomentumStrategy(lookback=20)
        }

        def market_handler(event):
            symbol = event.data['symbol']
            if symbol in strategies:
                strategies[symbol].on_market_data(
                    event, portfolio, matching_engine, event_loop
                )
    ...
    def test_backtest_metrics_validation(self, market_data_generator):
        ...
        risk_metrics = RiskMetrics.calculate_all_metrics(
            portfolio.equity_curve
        )
