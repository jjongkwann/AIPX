...
class Settings(BaseSettings):
    ...
    kafka_topic_market_data: str = Field(
        default="market-data-raw", alias="KAFKA_TOPIC_MARKET_DATA"
    )
    kafka_topic_trading_signals: str = Field(
        default="trading-signals", alias="KAFKA_TOPIC_TRADING_SIGNALS"
    )
    kafka_topic_order_events: str = Field(
        default="order-events", alias="KAFKA_TOPIC_ORDER_EVENTS"
    )
    kafka_topic_executions: str = Field(
        default="executions", alias="KAFKA_TOPIC_EXECUTIONS"
    )
    kafka_consumer_group: str = Field(
        default="aipx-consumer-group", alias="KAFKA_CONSUMER_GROUP"
    )
    ...
    postgres_password: str = Field(
        default="aipx_dev_password", alias="POSTGRES_PASSWORD"
    )
    postgres_db: str = Field(default="aipx", alias="POSTGRES_DB")
    postgres_max_connections: int = Field(
        default=20, alias="POSTGRES_MAX_CONNECTIONS"
    )
    ...
    timescaledb_password: str = Field(
        default="aipx_dev_password", alias="TIMESCALEDB_PASSWORD"
    )
    ...
    max_daily_loss_percent: float = Field(
        default=5.0, alias="MAX_DAILY_LOSS_PERCENT"
    )
    max_loss_per_trade_percent: float = Field(
        default=2.0, alias="MAX_LOSS_PER_TRADE_PERCENT"
    )
    ...
    enable_prometheus_metrics: bool = Field(
        default=True, alias="ENABLE_PROMETHEUS_METRICS"
    )
    prometheus_port: int = Field(default=9090, alias="PROMETHEUS_PORT")
    enable_jaeger_tracing: bool = Field(
        default=False, alias="ENABLE_JAEGER_TRACING"
    )
